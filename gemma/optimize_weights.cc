...
void Run(Args& args) {
  ...
  RandInit(ftiny->layers[0].bf_pre_ffw_rms_out, gen);
  ...
  for (; steps < 10000; ++steps) {
    ...
    ZeroInit(btiny->layers[0].bf_pre_ffw_rms_out);
    ...
    Update(btiny->layers[0].bf_pre_ffw_rms_out, scale,
           ftiny->layers[0].bf_pre_ffw_rms_out);
    printf("total_loss: %f\n", total_loss);
    if (total_loss < 0.01f) {
      break;
    }
