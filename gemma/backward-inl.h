...
void LayerVJP(const Layer<float, TConfig>& weights,
  ...
  // This can't be multi-threaded the ususal way because all threads would
  // write concurrently to the value part of backward.qkv
  ...
  // This can't be multi-threaded the ususal way because all threads would
  // write concurrently to the key part of backward.qkv
  for (size_t head = 0; head < kHeads; ++head) {
    for (size_t pos = 0; pos < num_tokens; ++pos) {
